...
blob_service = BlobServiceClient.from_connection_string(conn_str="connection string")

try:
    container_client = blob_service.get_container_client(container="container name")  # Connecting to Azure Blob Storage and accessing the container used to store user images
    ...
    container_client = blob_service.create_container("container name")                # https://www.youtube.com/watch?v=wToHU8Hts9c
    ...
    if request.method == 'POST':
        ...
        if 'password-recovery'  in request.form:                                          
            ...
            if user:                                                                        # a unique 16 digit token, encrypting it, and storing it in the database for later purposes.
                ...
                message = Message('Reset Password', sender='email', recipients=[recoveryEmail])
