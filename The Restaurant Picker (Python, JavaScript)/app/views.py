...
def home():
    if current_user.is_authenticated:   
        #geoip_data = simple_geoip.get_geoip_data()                 # Checking to see if the current user is authenticated to determine whether or not to display 'liked' restaurants.                           
        geoip_data = {'location':{'city':'Plattsburgh'}}            # Here, the API request to GeoIPify is commented out in order to save the limited number of requests that come with the free trial.
        ...
        for response in queryResponses:
            if(response.response_type == 'regional-response' or 
               response.response_type == 'foodtype-response' or 
               response.response_type == 'restaurantstyle-response' or 
               response.response_type == 'dietary-response'):
                temp = json.loads(response.responses)
                PARAMETERS['categories'].append(temp[1])
            elif response.response_type == 'price-response':
                temp = json.loads(response.responses)
                PARAMETERS['price'] = temp[1]
            elif response.response_type == 'accessibility-response':
                temp = json.loads(response.responses)
                PARAMETERS['attributes'] = temp[1]
        response = requests.get(url=BUSINESS_SEARCH,                        
                                params=PARAMETERS,                          
                                headers=HEADERS)                            
        parsed = json.loads(response.text)                                  
        recommendedBusinesses = parsed["businesses"]
        for business in recommendedBusinesses.copy():
            if Preferences.query.filter_by(user_id=current_user.id, business_id=business['id'], dislikes=True).first():
                recommendedBusinesses.remove(business)
        ...
        return render_template("home.html", user=current_user, userImageURL=get_img_url_with_blob_sas_token(current_user.userImage), trendingBusinesses=trendingBusinesses, allBusinesses=allBusinesses, likedRestaurants=likedRestaurants, recommendedBusinesses=recommendedBusinesses, location=geoip_data['location']['city'])
