export default class TurnipPlot{
    constructor(x, y, timePlanted){
        this.x = x;
        this.y = y;
        this.plotImage = document.getElementById('plot');
        this.cropImage = document.getElementById('turnipSpritesheet');
        this.itemImageId = "turnipItem";
        this.imageWidth = 82;
        this.imageHeight = 82;
    ...
    update(){
        var now = Date.now();
        if(this.stage < 6){
            if(this.timePlanted + (10000 * this.stage) < now){
                this.frameX += 1;
                this.stage += 1;
            }
        }
