export default class CarrotPlot{
    constructor(x, y, timePlanted){
        this.x = x;
        this.y = y;
        this.plotImage = document.getElementById('plot');
        this.cropImage = document.getElementById('carrotSpritesheet');
        this.itemImageId = "carrotItem";
        this.imageWidth = 82;
        this.imageHeight = 82;
    ...
    update(){
        var now = Date.now();
        if(this.stage < 6){
            if(this.timePlanted + (50000 * this.stage) < now){
                this.frameX += 1;
                this.stage += 1;
            }
        }
